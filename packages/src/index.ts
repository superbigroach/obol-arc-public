...
function client(): GatewayClient {
  ...
  return new GatewayClient({ chain: "arcTestnet", privateKey: key as Hex });
}
...
server.tool(
  "find_service",
  "Search the Obol directory for metered APIs an agent can pay for per call. Returns id, price (USDC), description, and the callable URL.",
...
server.tool(
  ...
  { amount: z.string().describe("USDC amount to deposit, e.g. '5'") },
  async ({ amount }) => {
    try {
...
server.tool(
  "pay_and_call",
  "Pay for and call an Obol metered service via Circle Gateway. Pass the service's callUrl from find_service. Returns the API result and the settlement tx.",
  {
